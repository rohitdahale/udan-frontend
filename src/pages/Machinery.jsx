...
const toolroomMachines = [
  { name: "Blohm Simplex 7 Surface Grinder", units: 1, icon: Settings, image: blohmGrinder },
  { name: "Lathe Machine", units: 1, icon: Wrench, image: latheMachine },
  { name: "Vertical Milling Machine", units: 1, icon: Zap, image: millingMachine },
  { name: "VMC BFW BMV 50 TC24", units: 1, icon: Cpu, image: vmcBfw },
  { name: "VMC Haas VF-2", units: 1, icon: Cpu, image: vmcHaas },
  { name: "Wirecut EDM ER 600", units: 1, icon: Target, image: wirecutEdm },
  { name: "EDM Machine", units: 1, icon: Zap, image: edmMachine },
  { name: "EDM Hole Driller", units: 1, icon: Target, image: edmHoleDriller },
];

const mouldingMachines = [
  { name: "Injection Moulding Machine 150 Ton", units: 2, icon: Layers, image: molding150 },
  { name: "Electronica Injection Moulding 200 Ton", units: 3, icon: Layers, image: molding200 },
  { name: "Electronica Injection Moulding 250 Ton", units: 1, icon: Layers, image: molding250 },
];
            ...
            {toolroomMachines.map((machine, index) => {
              ...
              return (
                <Reveal key={machine.name} delay={index * 0.05}>
                  <div className="group relative overflow-hidden rounded-2xl border border-hairline bg-[#020c06] h-72 cursor-pointer shadow-sm hover:shadow-xl transition-all duration-300">
                    <img
                      src={machine.image}
                      alt={machine.name}
                      loading="lazy"
            ...
            {mouldingMachines.map((machine, index) => {
              ...
              return (
                <Reveal key={machine.name} delay={index * 0.08}>
                  <div className="group relative overflow-hidden rounded-[2rem] border border-hairline bg-[#020c06] h-80 cursor-pointer shadow-lg hover:shadow-2xl transition-all duration-500">
                    <img
                      src={machine.image}
                      alt={machine.name}
                      loading="lazy"
